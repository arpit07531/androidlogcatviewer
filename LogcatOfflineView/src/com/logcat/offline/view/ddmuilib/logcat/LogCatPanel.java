...
public final class LogCatPanel implements ILogCatMessageEventListener, ILogCatSyncListener {
	...
	private static final String RESET_HIGHT_LIGHT = "Reset Highlight";
	private static final String RESET_THREAD_PROCESS_NAME = "Reset Thread/Process Name";
	...
	private static final String ACTION_HIGHLIGHT_TID = "Highlight Selected TID(s)";
	private static final String ACTION_SET_TID_FRIENDLY_NAME = "Set Thread Name";
	private static final String ACTION_SET_PID_FRIENDLY_NAME = "Set Application Name";
	...
	private Action mHighlightSelectedTID;
	private Action mResetPID;
	private Action mSetThreadName;
	private Action mSetProcessName;
	...
	private void addNewFilter() {
		...
		LogCatFilter f = new LogCatFilter(d.getFilterName().trim(), d.getTag().trim(), d.getText().trim(),
				d.getPid().trim(), "no tid",
				// d.getAppName().trim(),
	...
	private void editSelectedFilter() {
		...
		LogCatFilter f = new LogCatFilter(dialog.getFilterName(), dialog.getTag(), dialog.getText(), dialog.getPid(),
				"no tid",
				// dialog.getAppName(),
	...
	/*  *//**
			 ...
			 * exists, then create one and then select that. This method should be called
			 * from the UI thread.
			 * 
			 * @param appName application name to filter by
			 */
	/*
	 * public void selectTransientAppFilter(String appName) { assert
	 * mViewer.getTable().getDisplay().getThread() == Thread.currentThread();
	 * 
	 * LogCatFilter f = findTransientAppFilter(appName); if (f == null) { f =
	 * createTransientAppFilter(appName); mLogCatFilters.add(f); }
	 * 
	 * selectFilterAt(mLogCatFilters.indexOf(f)); }
	 * 
	 * private LogCatFilter findTransientAppFilter(String appName) { for
	 * (LogCatFilter f : mLogCatFilters) { if (f.isTransient() &&
	 * f.getAppName().equals(appName)) { return f; } } return null; }
	 * 
	 * private LogCatFilter createTransientAppFilter(String appName) { LogCatFilter
	 * f = new LogCatFilter(appName + " (Session Filter)", "", "", "", appName,
	 * LogLevel.VERBOSE); f.setTransient(); return f; }
	 * 
	 ...
	 * Currently, this feature is incomplete: The UI elements are created, but they
	...
	private void createLiveFilters(Composite parent) {
		...
		showFiltersColumn
				.setImage(ImageLoader.getDdmUiLibLoader().loadImage(IMAGE_DISPLAY_FILTERS, toolBar.getDisplay()));
	...
	/**
	 * Display a {@link FileDialog} to the user and obtain the location for the log
	 * file.
	 * 
	...
	private List<LogCatMessageWrapper> getSelectedLogCatMessages() {
		...
		// Get items from the table's input as opposed to getting each table item's
		// data.
		// Retrieving table item's data can return NULL in case of a virtual table if
		// the item
	...
	private void createLogcatViewTable(Composite parent) {
		// The SWT.VIRTUAL bit causes the table to be rendered faster. However it makes
		// all rows
		// to be of the same height, thereby clipping any rows with multiple lines of
		// text.
		// In such a case, users can view the full text by hovering over the item and
		// looking at
		// the tooltip.
		...
		String[] properties = { "Level", "Time", "PID", "TID", // add tid
				"Application", "Time Elapsed(ms)", "Tag", "Text", };

		/**
		 * The sampleText for each column is used to determine the default widths for
		 * each column. The contents do not matter, only their lengths are needed.
		 */
		String[] sampleText = { "   ", " 00-00 00:00:00.0000 ", " 0000", " com.android.launcher", " 16200", " 0000", // add
																														// tid
				" SampleTagText++++",
		...
		// This is not strictly necessary, except that on WinXP, the rows showed up
		// clipped. So
		...
		mViewer.getTable().addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				/*
				 * String time =
				 * getSelectedLogCatMessages().get(0).getLogCatMessage().getTime(); if (time !=
				 * null && time.length() > 10) {// 04-08 13:13:43.851
	...
	private void createViewMenu() {
		...
		mmg.add(new Action(RESET_ALL_FILTER) {
			...
			public void run() {
				...
				mResetHighLight.run();
				mClearSearch.run();
				mResetThreadAndProcessName.run();
				updateAppliedFilters();
			}
		});
		...
		mmg.add(new Action(SEARCH) {
			@Override
			public void run() {
				InputDialog inputDialog = new InputDialog(Display.getCurrent().getActiveShell(), "Search and Highlight",
						"Find:", "", null);
		...
		// Highlight Selected TID
		mHighlightSelectedTID = new Action(ACTION_HIGHLIGHT_TID) {
			@Override
			public void run() {
				List<LogCatMessageWrapper> selectedItems = getSelectedLogCatMessages();
				if (selectedItems == null || selectedItems.size() == 0) {
				...
				List<LogCatMessageWrapper> allItems = getAllLogcatMessageUnfiltered();
				setText(getText() + " : ");
				List<String> selectedTIDs = new ArrayList<String>();

				for (LogCatMessageWrapper item : selectedItems) {
					String selectTID = item.getLogCatMessage().getThreadFriendlyName();
					if (!selectedTIDs.contains(selectTID)) {
						setText(getText() + selectTID + ", ");
						selectedTIDs.add(selectTID);
					}
				}
				for (int i = 0; i < allItems.size(); i++) {
					LogCatMessageWrapper logCatMessageWrapper = allItems.get(i);
					String localTID = logCatMessageWrapper.getLogCatMessage().getThreadFriendlyName();
					if (selectedTIDs.contains(localTID)) {
						logCatMessageWrapper.setHighlight(true);
					}
				}
				mViewer.refresh();
		...
		// Set Thread name
		mSetThreadName = new Action(ACTION_SET_TID_FRIENDLY_NAME) {
			@Override
			public void run() {
				InputDialog dlg = new InputDialog(Display.getCurrent().getActiveShell(), "",
						"Enter Thread name (5-8 charcters)", "", new IInputValidator() {
				...
				if (dlg.open() == Window.OK) {
					...
					String threadName = dlg.getValue();
					...
					for (LogCatMessageWrapper item : selectedItems) {
						String selectTID = item.getLogCatMessage().getThreadFriendlyName();
						if (!selectedTIDs.contains(selectTID)) {
							setText(getText() + selectTID + "(" + threadName + "), ");
							selectedTIDs.add(selectTID);
						}
					}
					for (int i = 0; i < allItems.size(); i++) {
						...
						String localTID = logCatMessageWrapper.getLogCatMessage().getThreadFriendlyName();
						if (selectedTIDs.contains(localTID)) {
							logCatMessageWrapper.getLogCatMessage().setThreadFriendlyName(threadName);
						}
					}
					mViewer.refresh();
				}
		...
		mSetProcessName = new Action(ACTION_SET_PID_FRIENDLY_NAME) {
			@Override
			public void run() {
				InputDialog dlg = new InputDialog(Display.getCurrent().getActiveShell(), "",
						"Enter Application name (5-8 charcters)", "", new IInputValidator() {
				...
				if (dlg.open() == Window.OK) {
					// User clicked OK; update the label with the input
					String applicationName = dlg.getValue();
					...
					for (LogCatMessageWrapper item : selectedItems) {
						String selectedPid = item.getLogCatMessage().getPid();
						if (!selectedPIDS.contains(selectedPid)) {
							setText(getText() + selectedPid + "(" + applicationName + "), ");
					...
					for (int i = 0; i < allItems.size(); i++) {
						...
						String localPid = logCatMessageWrapper.getLogCatMessage().getPid();
						if (selectedPIDS.contains(localPid)) {
							logCatMessageWrapper.getLogCatMessage().setApplicationFriendlyName(applicationName);
						}
					}
					mViewer.refresh();
				}
		...
		mResetThreadAndProcessName = new Action(RESET_THREAD_PROCESS_NAME) {
			...
			public void run() {
				...
				for (int i = 0; i < allItems.size(); i++) {
					LogCatMessageWrapper logCatMessageWrapper = allItems.get(i);
					logCatMessageWrapper.getLogCatMessage()
							.setThreadFriendlyName(logCatMessageWrapper.getLogCatMessage().getTid());
					
					logCatMessageWrapper.getLogCatMessage().setApplicationFriendlyName(logCatMessageWrapper.getLogCatMessage().getAppName());
				}
				mViewer.refresh();
			}
		};
		...
		mmg.add(mHighlightSelectedTID);
		mmg.add(new Separator());
		mmg.add(mResetThreadAndProcessName);
		mmg.add(mSetProcessName);
		mmg.add(mSetThreadName);
	...
	/**
	 * Setup to automatically enable or disable scroll lock. From a user's
	 * perspective, the logcat window will:
	 * <ul>
	 * <li>Automatically scroll and reveal new entries if the scrollbar is at the
	 * bottom.</li>
	 * <li>Not scroll even when new messages are received if the scrollbar is not at
	 * the bottom.</li>
	 * </ul>
	 * This requires that we are able to detect where the scrollbar is and what
	 * direction it is moving. Unfortunately, that proves to be very platform
	 * dependent. Here's the behavior of the scroll events on different platforms:
	 * <ul>
	 * <li>On Windows, scroll bar events specify which direction the scrollbar is
	 * moving, but it is not possible to determine if the scrollbar is right at the
	 * end.</li>
	 * <li>On Mac/Cocoa, scroll bar events do not specify the direction of movement
	 * (it is always set to SWT.DRAG), and it is not possible to identify where the
	 * scrollbar is since small movements of the scrollbar are not reflected in
	 * sb.getSelection().</li>
	 * <li>On Linux/gtk, we don't get the direction, but we can accurately locate
	 * the scrollbar location using getSelection(), getThumb() and getMaximum().
	 * </ul>
	 */
	private void setupAutoScrollLockBehavior() {
		if (DdmConstants.CURRENT_PLATFORM == DdmConstants.PLATFORM_WINDOWS) {
			...
			// On Linux/gtk, we do not get any details regarding the scroll event
			// (up/down/etc).
			...
			// nor can we rely on getSelection() changing for small movements. As a result,
			// we
			// do not setup any auto scroll lock behavior. Mac users have to manually pause
			// and
			// unpause if they are looking at a particular item in a high volume stream of
			// events.
		}
	}
	...
	/**
	 * Perform all necessary updates whenever a filter is selected (by user or
	 * programmatically).
	 */
	private void filterSelectionChanged() {
		...
		if (idx == -1) {
			/*
			 * One of the filters should always be selected. On Linux, there is no way to
			 * deselect an item. On Mac, clicking inside the list view, but not an any item
			 * will result in all items being deselected. In such a case, we simply reselect
			 * the first entry.
			 */
			idx = 0;
			mFiltersTableViewer.getTable().setSelection(idx);
	...
	private List<LogCatViewerFilter> getCurrentLiveFilters() {
		...
		List<LogCatFilter> liveFilterSettings = LogCatFilter.fromString(mLiveFilterText.getText(), /* current query */
				LogLevel.getByString(mCurrentFilterLogLevel), mSelectedPIDList,
				mSelectedTagList); /* current log level */
	...
	/**
	 * When new messages are received, and they match a saved filter, update the
	 * unread count associated with that filter.
	 * 
	...
	/**
	 * Refresh the logcat table asynchronously from the UI thread. This method adds
	 * a new async refresh only if there are no pending refreshes for the table.
	 * Doing so eliminates redundant refresh threads from being queued up to be run
	 ...
	 * List<LogCatMessage> selectedMessages = getSelectedLogCatMessages(); if
	 * (selectedMessages.size() == 0) { return; }
	 * 
	...
	private Action mResetThreadAndProcessName;
	private Action mClearSearch;

	/**
	 * Specify the listener to be called when the logcat view gets focus. This
	 * interface is required by DDMS to hook up the menu items for Copy and Select
	 * All.
	 * 
